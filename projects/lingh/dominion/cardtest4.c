...
void testPlayVillage() {
    ...
    //Initialize hands
    //Player 1
    //Hand [0] with village and the first 4 kingdom cards
    state.handCount[0] = 0;
    ...
    printf("TEST 1 - Current player receives exactly 1 card: ");
    assertTrue(state.handCount[0], tempHandSize);

    //TEST 2: The received card should come from the player's own cards
    printf("TEST 2 - The received card comes from player's own cards: ");
    assertTrue(state.deckCount[0]+state.discardCount[0], tempDeckSize+tempDiscardSize);
    ...
    //TEST 5: The current player receives an additional two actions
    printf("TEST 5 - The current player receives two additional actions: ");
